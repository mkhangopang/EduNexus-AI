...
import { Save, RefreshCw, AlertTriangle, TrendingUp, ThumbsUp, AlertCircle, ArrowRight, BrainCircuit, Check } from 'lucide-react';
import { getSystemInstruction, saveSystemInstruction, resetSystemInstruction } from '../services/geminiService';
...
export const BrainControl: React.FC = () => {
    ...
    useEffect(() => {
        setPrompt(getSystemInstruction());
    }, []);
    ...
    const handleReset = () => {
        if (window.confirm("Are you sure you want to reset the Neural Core to factory defaults? This will erase all your pedagogical customizations.")) {
            const defaultPrompt = resetSystemInstruction();
        ...
        <div className="p-6 space-y-8 h-full overflow-y-auto">
                ...
                <div className="flex gap-3">
                    ...
                    <button 
                        ...
                        className={`flex items-center gap-2 px-4 py-2 text-white rounded-lg transition-all shadow-sm ${
                            showSuccess ? 'bg-emerald-600' : 'bg-indigo-600 hover:bg-indigo-700'
                        }`}
                    >
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-amber-700">Changes here affect the AI behavior immediately for all your chats. Please test carefully.</p>
                </div>
            </div>

            {/* Editor Section */}
            <div className={`bg-white rounded-xl shadow-sm border overflow-hidden flex flex-col h-[500px] transition-colors ${isDirty ? 'border-indigo-300' : 'border-slate-200'}`}>
                <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        <span className="text-xs font-mono text-slate-500">system_instruction.txt</span>
                        {isDirty && <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-bold">UNSAVED</span>}
                    </div>
